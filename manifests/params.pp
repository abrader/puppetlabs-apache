...
class apache::params inherits ::apache::version {
  ...
  if $::osfamily == 'RedHat' or $::osfamily == 'suse' or $::operatingsystem == 'amazon' {
    $fm_instance_name     = hiera('fm_instance_name', 'demo_run')
    $user                 = 'nsbin1'
    $group                = 'nsgroup'
    $root_group           = 'root'
    $apache_name          = 'apache2'
    $service_name         = 'apache2'
    $httpd_dir            = "/export/appl/website/apache/${fm_instance_name}"
    $server_root          = '/export/appl/website/apache'
    $conf_dir             = "${httpd_dir}/config"
    $confd_dir            = "${httpd_dir}/config"
    $mod_dir              = "${httpd_dir}/config"
    $mod_enable_dir       = undef
    $vhost_dir            = "${httpd_dir}/config"
